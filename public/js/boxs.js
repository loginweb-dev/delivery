(function (name, context, definition) {
	...
	PromptBoxes.prototype = {
	   ...
	   */
	  prompt: async function (callback, msg, inputType, value, submitText, cancelText, opts) {
		var that = this;
		...
		switch (opts.inputType) {
		  case 'textarea':
			...
			break;
  		  case 'select':
			var milocationes = await axios('https://delivery.appxi.net/api/poblaciones')
			input$ = document.createElement('select');
			var array = [{id: 1, nombre: 'A'}, {id: 2, nombre: 'B'}];
			var option = document.createElement("option");
			option.value = 0;
			option.text = 'Elije una opcion';
			input$.appendChild(option);
			for (var i = 0; i < milocationes.data.length; i++) {
				var option = document.createElement("option");
				option.value = milocationes.data[i].id;
				option.text = milocationes.data[i].nombre;
				input$.appendChild(option);
			}
			break;
		  default:
